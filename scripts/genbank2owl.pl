...
genbank2owl.pl -- converts GenBank files to OWL

=head1 SYNOPSIS

  genbank2owl.pl [options] filename(s)

  # process a directory containing GenBank flatfiles
  genbank2owl.pl --dir path_to_files --zip

  # process a single file, ignore explicit exons and introns
  perl genbank2owl.pl --filter exon --filter intron file.gbk.gz

  # process a list of files 
  perl genbank2owl.pl *gbk.gz
  ...
  | perl genbank2owl.pl -noCDS -in stdin -out stdout \
  | perl gmod_bulk_load_gff3.pl -dbname mychado -organism fromdata

    Options:
        --enumerate-junctions
                  -j  materialize individuals for all interbase junctions
...
Several other improvements and bugfixes, minor but useful are included
  ...
  * IO pipes now work:
    curl ftp://ncbigenomes/... | genbank2owl --in stdin --out stdout | gff2chado ...
...
unflattner/genbank2 doesn't
 ...
 Subject: genbank2owl.PLS and the unflatenner - Inconsistent	order?
...
sub gff_header {
    ...
    $head .= "# conversion-by bp_genbank2owl.pl\n";
    if($source_feat) {
